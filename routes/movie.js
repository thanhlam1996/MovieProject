...
var s3upload = multer({
  storage: multerS3({
    s3: s3,
    bucket: dynamoDbConfig.bucketName,
        ...
        var params = {
          ...
          Item: {
            ...
            process: {
              create: {
                creater: [
                  req.session.passport.user.fullname,
  ...
  if (CheckLogin(2, res, req)) {
    ...
    docClient.query(params, function(err, data) {
      if (err) {
        ...
        if (data.Count > 0) {
          return res.render("../views/movies/admin-update-movie.ejs", {
            data,
            moment: moment
          });
        }
...
router.get("/list-movie-registed", function(req, res, next) {
  if (CheckLogin(2, res, req)) {
    ...
    var params = {
      ...
      FilterExpression:
        "(#stt=:stt) AND (process.registion.register[1]=:email) ",
      ExpressionAttributeNames: {
        "#stt": "stt"
      },
      ExpressionAttributeValues: {
    ...
    docClient.scan(params, function(error, result) {
      if (error) {
        ...
        res.render("../views/movies/list-movie-registed.ejs", {
          result,
          moment: moment
        });
      }
...
router.post("/member-register-movie", function(req, res, next) {
  if (CheckLogin(2, res, req)) {
    ...
    var params = {
      ...
      UpdateExpression: "set stt=:st, process.registion=:wr",
      ExpressionAttributeValues: {
        ":st": 3,
        ":wr": {
  ...
  if (CheckLogin(2, res, req)) {
    ...
    docClient.query(params, function(err, data) {
      if (err) {
        ...
        if (data.Count > 0) {
          return res.render("../views/movies/member-writing-movie.ejs", {
            data,
            moment: moment
          });
        }
  ...
  if (CheckLogin(3, res, req)) {
    ...
    docClient.scan(params, function(error, result) {
      if (error) {
        ...
        res.render("../views/movies/admin-approve-movie.ejs", {
          result,
          moment: moment
        });
      }
  ...
  if (CheckLogin(2, res, req)) {
    ...
    docClient.scan(params, function(error, data) {
      if (error) {
        ...
        res.render("../views/movies/list-approving-member.ejs", {
          data,
          moment: moment
        });
      }
...
router.post("/unapprove-movie-admin", function(req, res, next) {
  if (CheckLogin(3, res, req)) {
    ...
    var params = {
      ...
      UpdateExpression: "set process.approve.complaint=:a",
      ExpressionAttributeValues: {
        ":a": complaint
      },
      ReturnValues: "UPDATED_NEW"
...
router.post("/delete-movie-admin", function(req, res, next) {
  if (CheckLogin(3, res, req)) {
    ...
    if (role > 2) {
      ...
      docClient.query(param1s, function(err, result) {
        //lay file hinh
        if (err) {
          console.error(
            "Unable to read item. Error JSON:",
          ...
          if (stt == 4 || stt == 3) {
            ...
            s3Fs.unlink(img, function(err, data) {
              //Xoa img s3
              if (err) {
                throw err;
              } else {
  ...
  if (CheckLogin(3, res, req)) {
    ...
    docClient.query(params, function(err, data) {
      if (err) {
        ...
        return res.render("../views/movies/admin-update-content-movie.ejs", {
          data,
          moment: moment
        });
      }
...
// =========== role=2
router.post("/update-movie-member", s3upload.single("posterimage"), function(
  req,
  res,
  next
) {
  if (CheckLogin(2, res, req)) {
    ...
    s3Fs.unlink(imgold, function(err, data) {
      //Xoa img s3
      if (err) {
        throw err;
      } else {
    ...
    if (CheckLogin(3)) {
      ...
      if (imgname == "") {
        var params = {
          ...
          UpdateExpression:
            "set title=:g, info.producer=:f, info.director=:a, info.distance=:b, info.publicationdate=:c, info.actor=:d, info.movietype=:e, info.trailer=:h, info.content=:i, info.country=:l",
          ExpressionAttributeValues: {
            ":a": director,
            ":b": distance,
            ":c": publicationdate,
        ...
        var params = {
          ...
          UpdateExpression:
            "remove title, info.producer, info.directo, info.distance, info.publicationdate, info.actor, info.movietype, info.posterimage, info.trailer, info.content,  info.country",
          ReturnValues: "UPDATED_NEW"
        };
      }
          ...
          docClient.update(params, function(err, data) {
            if (err) {
              ...
              var params = {
                ...
                UpdateExpression:
                  "set title=:k, info.producer=:f, info.director=:a, info.distance=:b, info.publicationdate=:c, info.actor=:d, info.movietype=:e, info.posterimage=:g, info.trailer=:h, info.content=:i,  info.country=:l",
                ExpressionAttributeValues: {
                  ":a": director,
                  ":b": distance,
                  ":c": publicationdate,
  ...
  if (CheckLogin(2, res, req)) {
    ...
    docClient.scan(params, function(error, data) {
      if (error) {
        ...
        res.render("../views/movies/list-movie-member-writed.ejs", {
          data,
          moment: moment
        });
      }
  ...
  if (CheckLogin(3, res, req)) {
    ...
    docClient.scan(params, function(error, data) {
      if (error) {
        ...
        res.render("../views/movies/manager-movie-admin.ejs", {
          data,
          moment: moment
        });
      }
      ...
      if (err) {
        ...
        if (isexits) {
          ...
          var params = {
            ...
            UpdateExpression: "set #comment=:cmt, #ratings=:rat",
            ExpressionAttributeNames: {
              "#comment": "comment",
              "#ratings": "ratings"
            },
      ...
      if (err) {
        ...
        if (isexits) {
          ...
          var params = {
            ...
            UpdateExpression: "set #like=:like, #countlike=:cnt",
            ExpressionAttributeNames: {
              "#like": "liker",
              "#countlike": "countlike"
            },
